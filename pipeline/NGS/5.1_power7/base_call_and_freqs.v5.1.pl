...
#########################################################################
# Parses Blast (-outfmt 6, btop format) of reference seqeunce against cirSeq reads
...
# V3.1.1: bug fix, skip read positions that map to several locations in the genome.
# V3.2: remove reads where<85% of the read does not map to the genome
# V4: change in processing: use btop format in blast, and process quality scores directly in this script (no prefiltering). require all repeats to have high quality
# V4.1: allow skipping gaps and do not base call at ends of reads
# V5: quality_cutoff = 23; read quality taken from corresponding part's quality file; printing all contributing mutations
#########################################################################

die "usage base_call_and_freqs.pl <blast output file in -m6 btop format format> <fastq (or fastq.gz) with quality scores> <reference sequence used with blast, fasta format> <output file for frequencies> <infer gaps? Y/N default Y> <min number repeats; default=2> <quality cutoff; default=23, requires all repeats to have this q or higher> <ascii table quality scores, defaulted at /scratch300/winter/data/ascii_table_processed.txt> " unless (scalar(@ARGV)>=4);

my $power7_version_path = "/sternadi/home/volume1/shared/tools/pipelineNGS/5.1_power7";
...
if (defined $ARGV[5]) {
    $min_for_calling=$ARGV[5];
}

#my $quality_cutoff=20;
my $quality_cutoff=23;
if (defined $ARGV[6]) {
    $quality_cutoff=$ARGV[6];
}

my $end_read_limit = 200; # from here we requrie higher stringency and suspect Illmina derived errors
#my $ascii_q_file = "/scratch300/winter/data/ascii_table_processed.txt";
my $ascii_q_file = "$power7_version_path/ascii_table_processed.txt";
...
my %relevant_reads;
my %ref;
my $perc_mapping_required=0.85;
my $read_end_threshold = 200; # from 200 enowards I consider this the end of the read, more prone to Illumina errors 
...
sub process_reads {
	...
	# remove alignments which contain N's
	#next if ($aln =~ /N/);
	...
	for my $i(1..$aln_length) {
		...
		unless ($read_position > $read_end_threshold) { ## do not basecall a mutation (diff from ref) mapped at the end of the read
...
## I treat ends of reads the same as middle of reads
...
## These cases are removed from the tally
    my $rem_double_map=0;
    my $reads_filtered_threshold_mapping=0;
    foreach my $read_id (keys %reads_processed) {
	my $read_mapped_length=$reads_edges{$read_id}{NUM_BASES};
	my $read_proxy_length=$reads_edges{$read_id}{END};
# filteration for >85% of read mapping to genome
	if ($read_mapped_length < $read_proxy_length * $perc_mapping_required ) {
	    $reads_filtered_threshold_mapping++;
	    ...
	    delete $reads_processed{$read_id};
	}
## filteration for double mapping
    ...
    print SUMMARY $reads_filtered_threshold_mapping. " reads removed that did not map >85% of read\n";
}
...
#grep -f ids.txt -F --mmap -A 3 seq.fq | grep -v '^--'
    ...
    #for ( my $i=0; $i<scalar(@lines)-1; $i=$i+4) {
	for ( my $i=0; $i<scalar(@lines)-1; $i=$i+2) {    
	my $line=$lines[$i];
	if ($line =~ m/\@(\S+)/) { #removed a space in the regex since it does not always exist...?
	    my $read=$1;
	    #my $q_line = $lines[$i+3];
	    my $q_line = $lines[$i+1];
...
## here: need to obtain read locations, and grep out the Q score; note we have to add a space for the search for grep otherwise many matches come up (e.f., 1443 will come up for 144)
		...
		if ($num_repeats >= $min_for_calling ) {
		    my $quality_counter=0;
		    my $avg_q_score=1; #MAOZ: Shouldn't it be initialized to 0??
			...
			if ($read_position >= $end_read_limit) {
			    $num_repeats--;
			}
			...
			if ($asciiScores{$q_val}>=$quality_cutoff) {
			    $quality_counter++;
			}
			...
			if ($avg_q_score>=$quality_cutoff) {
#		    if ($quality_counter >= $min_for_calling) { # enough with high quality
			...
			print SUMMARY "multiple repeats $num_repeats, mutation removed due to low Q (only $quality_counter high Q scores): read id $read pos at ref $pos base $base ref ".$ref{$pos}."\n" if ($ref{$pos} ne $base);
		    }
		}  #if num_repeats>= $min_for_calling 
	    } #base
	} #read
...
#	    $freq = sprintf "%.6f", $freq;
	    }
#	print OUT $pos."\t".$base."\t".$freq."\t".$reference{$pos}."\t".$base_counts{$pos}."\n";
	}
