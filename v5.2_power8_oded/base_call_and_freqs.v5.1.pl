...
my $end_read_limit = 500; # from here we requrie higher stringency and suspect Illmina derived errors
...
my $read_end_threshold = 500; # from 200 enowards I consider this the end of the read, more prone to Illumina errors 
...
# in the next line ommitted --mmap because apparently not suuported 20160827 Tal
	my $cmd_line= "$grep_name -F -A3 -f $list_reads $fastq_file \| grep -v \'^--\' > $list_reads_q"; 
    my $res=`$cmd_line`;
