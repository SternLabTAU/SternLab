...
def main(args):
    ...
    only_once_reads = subprocess.check_output("grep -P '^1\t' *stats  -h | awk '{sum+=$2}END{print sum}'", shell=True)
    #number of reads that are "contributing to frequency counts”
    freq_contr = subprocess.check_output("grep 'reads contributing to frequency counts' -h *stats | awk '{sum+=$1}END{print sum}'", shell=True)
    #number of bases called
    num_based_called = subprocess.check_output(
        "grep 'num bases called' *stats | awk -F = '{sum+=$2}END{print sum}'", shell=True)
    ...
    freq_file_path = os.path.join(input_dir, [f for f in os.listdir(input_dir) if f.endswith("freq")][0])
