...
# V5.3
# merged CirSeq pipeline with NGS pipeline runners and auxiliary functions
# 
#
#
#########################################################################

die "usage pipeline_runner.pl  <input directory with fastq.gz or fastq files> <output dir> <reference genome seq (fasta)>\
<start at step number> <end at step number> <type of input files, optional f if fastq and not zipped files> <refer to gaps? Y/N default Y> <NGS/Cirseq? type 2 for Cirseq\
(min num repeats=2) or 1 for NGS (min num repeats=1)> <Q-score cutoff, default =23> <% id for blast, default=85>\n
...
print "fasta cmd1: $cmd1\n";
    ...
    my $cmd2="";
    if ($type_file eq "z") {
		$cmd2="perl $scripts_dir/toFastaAndSplit_v5.3.pl $in_dir\$INFILE.fastq.gz $out_dir\$INFILE $number_of_reads_per_fasta_file\n";
    } else {
		$cmd2="perl $scripts_dir/toFastaAndSplit_v5.3.pl $in_dir\$INFILE.fastq $out_dir\$INFILE $number_of_reads_per_fasta_file f\n";
	}
...
print "base_call: list_parts_fasta_files:".$list_blast_results_files;
    ...
    my $cmd3="perl $scripts_dir/base_call_and_freqs.v5.3.pl \$INFILE \$FASTQ $ref_genome \$INFILE\.freqs $do_gaps $min_num_repeats $q_cutoff\n";
    my $mem_request = 8; 
...
print "join_files\n";
    ...
    my $cmd1="perl $scripts_dir/join_freqs_files.v5.3.pl $out_dir $output_file $do_gaps";
