...
use lib "/sternadi/home/volume1/shared/tools/pipelineNGS/5.1_power7";
use Create_cmd;
...
# 3. Run formatdb on each of N files above, and blast against the reference sequence
...
# V5.1.1
# updated sleep manager to be more efficient on finished jobs
#########################################################################


die "usage pipeline_runner.pl  <input directory with fastq.gz files> <output dir> <reference genome seq (fasta)> <error output file (with path)> <start at step number> <end at step number> <type of input files, optional f if fastq and not zipped files> <refer to gaps? Y/N default Y> <NGS/Cirseq? type 2 for Cirseq (default;min num repeats=2) or 1 for NGS (min num repeats=1)> <Q-score cutoff, default =23> <% id for blast, default=85>\n
...
my $min_num_repeats=2; # default runs CirSeq
if (defined $ARGV[8]) {
    $min_num_repeats=$ARGV[8];
    if ($min_num_repeats==1) {
	print "Running NGS mapping (not CirSeq mapping)\n";
    }
...
my $q_cutoff=23; # default quality cutoff per base is 23
...
my $scripts_dir = $power7_version_path;
...
#my $number_of_reads_per_fasta_file=500000; # temporaray change, bigmem
...
print "fasta cmd1: $cmd1\n";
    ...
    my $cmd2="";
    if ($type_file eq "z") {
		$cmd2="perl $scripts_dir/toFastaAndSplit_v5.1.pl $in_dir\$INFILE.fastq.gz $out_dir\$INFILE $number_of_reads_per_fasta_file\n";
    } else {
		$cmd2="perl $scripts_dir/toFastaAndSplit_v5.1.pl $in_dir\$INFILE.fastq $out_dir\$INFILE $number_of_reads_per_fasta_file f\n";
	}
...
# tal patch 2016-11-22 if only one file is present, ARRAY_INDEX is not defined
...
print "base_call: list_parts_fasta_files:".$list_blast_results_files;
    ...
    my $cmd3="perl $power7_version_path/base_call_and_freqs.v5.1.pl \$INFILE \$FASTQ $ref_genome \$INFILE\.freqs $do_gaps $min_num_repeats $q_cutoff\n";
...
sub test_qstat {
    my $job_num=shift;
    my $cmd = "qstat \| awk \'\$1 ~ \/^$job_num\/' \| sed \'s\/ \\+\/\\t\/g' \| cut -f5 \| grep -v C \| wc \| awk \'{print \$1}\'";
